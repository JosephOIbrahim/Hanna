...
| q006 | Idempotency-key shape for L4b — UUID, content hash, ULID, or `(phase, rounded-rhythm-time, body-hash)`? Without this, `publish(brief)` produces duplicate Joe-visible calendar events on every retry (scout-ops B-OPS-002; blocks L4b) | high | closed | c017 | 2026-05-25 |
| q007 | "Brief composition" boundary — when does one composition begin and end (per D001 "≤1 coach call per brief composition" + D005.1 `begin_composition` / `end_composition`)? Currently undefined in machine-checkable terms; today's PoC satisfies the rule "accidentally" (scout-architecture; blocks L6 mcp_tools) | high | closed | c032 | 2026-05-25 |
| q014 | Secret-storage substrate for `OverrideToken` HMAC — Mac keychain via `python-keychain`? `.env` file? environment variable? Blocks `OverrideToken` schema and Rule 34 override path (Phase 5 carried; DELIBERATE cycle 1 surfaced explicitly) | high | open | none | 2026-05-25 |
| q015 | `OverrideToken` HMAC key rotation — single static key (v1) vs rotating (v2)? Default deferral: v1 = single static key + documented limitation. Blocks `OverrideToken` v2 evolution (DELIBERATE cycle 1) | low | open | none | 2026-05-25 |
| q016 | Octavius reachability — does Octavius exist as a runnable subprocess on Joe's Mac today? Replaces the Line C sandbox spike (which DIED on the FORUM because L1 verifier inaccessible from this Linux env). Asking Joe directly is the verifier; blocks L7 octavius_bridge design | high | closed | c034 | 2026-05-25 |
| q017 | Octavius contract surfacing — how does Hanna learn Octavius's actual MCP-stdio request/response envelope? Three paths: (a) Joe pastes the contract verbatim; (b) widen `mcp__github__*` scope to include the Octavius repo; (c) Joe installs Octavius locally + runs it once + shares the API shape. Blocks `FormationRequest` + `FormationOutput` schema authoring (deferred from Line B cycle 1) and L7 implementation | high | closed-pending-action | c036 | 2026-05-25 |
...
| q009 | `docs/SESSION_01_RECON.md` §G + HANNA_DESIGN_ADOPTION.md status — correct, stamp historical, or retire? (scout-docs B3 + MAJOR — independently re-raised from q003) | low | closed | c025 | 2026-05-25 |
| q010 | Model-id rule scope — does CLAUDE.md's prohibition extend to docs/ and commit messages, or only to code? `REVIEW_2026-05-22.md` carries a model-id string today (scout-docs MAJOR) | low | closed | c026 | 2026-05-25 |
