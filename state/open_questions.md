...
| q013 | ORCHESTRATOR.md §7 resolution — amend §7 to permit per-GOAL `state/plan.md` as a standing file, or migrate the GOAL block to NEXT.md per the original adapter rule and retire `state/plan.md`? (scout-architecture MAJOR + scout-docs MAJOR; orchestrator self-violates on its first run) | high | open | none | 2026-05-25 |
